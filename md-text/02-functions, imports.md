...
Функція в python - об'єкт, який приймає аргументи та повертає значення. Зазвичай функція визначається за допомогою інструкції def.
...
**Якщо, Ви стикалися із функціями в такій мові програмування як C/C++, яка є статично типізованою ось деякі ключові моменти, які потрібно знати:**

- **Типи параметрів функції**

Оскільки python динамічно — типізована мова програмування і нам не потрібно вказувати тип параметра, туди передати ми можемо будь-що: Наприклад функція, яка, начебто повинна додавати 2 числа, несподівано може опинитися функцією конкатенації:
...
В даному випадку додати число зі строкою не можна, оскільки в нас відсутній статичний етап компіляції, то підказок не буде ніяких про помилку.
...
Оператор return, який зустрічається в тілі функції завершує її виконання, якщо поряд з оператором return буде об'єкт, його буде повернуто, в іншому випадку буде повернуто None. Давайте це продемонструємо:
...
Але зверніть увагу, що наприклад такий код, призведе до помилки:
...
Ми можемо передавати в функцію параметри не по порядку використовуючи іменовані параметри, наприклад:
...
Задача: написати функцію, яка приймає будь-яку кількість рядків конкатенує і повертає їх. Вирішити таку задачку допоможе rest - параметр: якщо поставити * перед іменем параметра — це означає взяти всі передані і зробити список всіх переданих параметрів.
...
Doc trings - рядкові літерали, які є першим оператором в модулі, функції, класі або визначенні методу. Такий рядковий літерал стає атрибутом __doc__ цього об'єкта.

Усі модулі мають, як правило, повинні містити doc strings, і всі функції та класи.
...
Повна документація функції містить:
- Пояснення функції
...
Вітаємо, ви знайшли «пасхальне яйце» в Python, також відоме як «Дзен». Це одна з кращих неофіційних частин роботи в Python. Сам по собі модуль this не робить нічого особливого, тільки показує оригінальний спосіб імпорту чогось. Тепер імпортуймо що-небудь, чим ми зможемо скористатися в майбутньому, наприклад, модуль math:
...
Ми імпортували модуль math і використали функцію sqrt, - це не єдина функція в цьому модулі та нам доступні вони всі використовуючи лише ''' import math '''
...
Поки що ми імпортували лише вбудовані модулі в Python. Напишімо власні. Нехай в нас буде така структура проекту:

![](../resources/img/2/6.png)

З наступним вмістом файлів:
...
Об'єкт datetime.date є три поля (year, day, month) які ми можемо прочитати та змінити:
```py
import datetime
...
Порівнювати дати в Python досить просто. Дати можна легко порівняти за допомогою операторів порівняння (наприклад, <,>, <=,> =,! = Тощо). Подивімося, як порівняти дати за допомогою модуля часу, використовуючи Python.
...
Найменший номер року, дозволений в об'єкті date, datetime. MINYEAR - це 1. Найбільше число років, дозволене в об'єкті date або datetime. MAXYEAR - 9999.
...
Створіть функцію і виконайте завдання згідно із варіантом. Документуйте функцію, використовуючи doc-string і підказки типів.

# Варіанти

1. Запросіть в користувача 3 числові значення(рік, число, місяць) і створіть об'єкт datetime.date. Перевірте, якщо введена дата відрізняється від сьогоднішньої не більше ніж на п'ять днів повідомте про це користувачеві.
2. Функція приймає час. Перевірте якщо переданий час був не більше 5 хвилин тому — напишіть 5 хвилин тому. Зробіть відповідно 10, 15, 20, 30 хвилин тому.
3. Запросіть у користувача дату. Перевірте, якщо введений місяць дати знаходиться в діапазоні маж вереснем і груднем повідомте про це користувача.
4. Запросіть в користувача 3 числові значення(рік, число, місяць) і створіть об'єкт datetime.date. Перевірте чи це робочий день понеділок-пя'тниця чи вихідний. Повідомте про це користувача.
5. Запросіть в користувача 2 дати(рік, число, місяць) і створіть об'єкти datetime.date. Визначте скільки годин різниці між цими датами та виведіть користувачеві.
6. Запросіть в користувача 5 дат(рік, число, місяць) і створіть об'єкти datetime.date. Визначіть найбільшу дату і повідомте про це користувача.
7. Запросіть в користувача час і створіть об'єкт datetime.time. Визначте скільки годин залишилося до кінця дня і повідомте про це користувача.
8. Запросіть в користувача час і створіть об'єкт datetime.time. Визначте чи час вказаний як ранок, обід, вечір і повідомте про це користувача.
9. Запросіть в користувача дату і час і створіть об'єкт datetime.datetime. Виведіть дату у форматі, наприклад, 2019 рік 21 вересня 12 годин і 1 хвилина.
...
7. Як можна імпортувати об'єкти з одного модуля в інший?
8. Перелічіть і поясніть класи із модуля datetime.